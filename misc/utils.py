...
def logger(exp_path, exp_name, work_dir, exception, resume=False):
    ...
    cfg_file = open('./config/train_config.py', "r")
...
def update_model(net, optimizer, scheduler, epoch, i_tb, exp_path, exp_name, scores, train_record, log_file=None,
    ...
    snapshot_name = 'all_ep_%d_mae_%.1f_mse_%.1f' % (epoch + 1, mae, mse)
    if type == 'extra':
        snapshot_name = 'e_all_ep_%d_mae_%.1f_mse_%.1f' % (epoch + 1, mae, mse)
    ...
    if mae < train_record['best_mae'] or mse < train_record['best_mse']:
        ...
        best_weight_path = os.path.join(exp_path, exp_name, snapshot_name + '.pth')
    ...
    last_weight_path = os.path.join(exp_path, exp_name, 'latest_state.pth')
    return train_record, best_weight_path, last_weight_path
...
def copy_cur_env(work_dir, dst_dir, exception):
    ...
    for filename in os.listdir(work_dir):
        if filename == 'RGB_IR' or filename == 'pre-trained' or filename == 'checkpoints':
            continue
