...
class RandomFlip(object):
    ...
    def __call__(self, img, mask, bbx=None):
        flip_mode = random.randint(0, 5)
        if type(img) is list:
            if random.random() < 0.5:
                return [img[0].transpose(flip_mode), img[1].transpose(flip_mode)], mask.transpose(flip_mode)
            return [img[0], img[1]], mask

        if random.random() < 0.5:
            if bbx is None:
                return img.transpose(flip_mode), mask.transpose(flip_mode)
            w, h = img.size
            xmin = w - bbx[:,3]
            xmax = w - bbx[:,1]
            bbx[:,1] = xmin
            bbx[:,3] = xmax
            return img.transpose(flip_mode), mask.transpose(flip_mode), bbx
        if bbx is None:
            return img, mask
        return img, mask, bbx
...
class Elastic_transform(object):
    ...
    def __call__(self, image, mask, elastic_value_x=0.0002,elastic_value_y=0.0002):
...
class RandomHorizontallyFlip(object):
    def __call__(self, img, mask, bbx=None):
        if type(img) is list:
            if random.random() < 0.5:
                return [img[0].transpose(Image.FLIP_LEFT_RIGHT), img[1].transpose(Image.FLIP_LEFT_RIGHT)], mask.transpose(Image.FLIP_LEFT_RIGHT)
            return [img[0], img[1]], mask
